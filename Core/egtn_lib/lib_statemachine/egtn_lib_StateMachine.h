...
typedef int (*SM_GuardFunc)(SM_StateMachine* self, void* pEventData);
...
// Private functions
void _SM_ExternalEvent(SM_StateMachine* self, const SM_StateMachineConst* selfConst, unsigned char newState, void* pEventData);
...
#define GUARD_DECLARE(_guardFunc_, _eventData_) \
    static int GD_##_guardFunc_(SM_StateMachine* self, _eventData_* pEventData);
...
#define END_TRANSITION_MAP(_smName_, _eventData_) \
    ...
    assert_param((sizeof(TRANSITIONS)/sizeof(uint8_t)) == (sizeof(_smName_##StateMap)/sizeof(_smName_##StateMap[0])));
